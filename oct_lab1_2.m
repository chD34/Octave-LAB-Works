...
N_fi = 10;
fi_values = linspace(-pi, pi, N_fi);  % генерація fi
...
for j = 1:N_fi
    ...
    time = l/v;
    plot(x_1_values, x_2_values, 'DisplayName', ['fi = ', num2str(fi), ', Час: ', num2str(time)], 'Color', colors(j, :));
...
title('Траєкторія переміщення човна залежно від fi', 'FontSize', 20);
grid on;
set(gca, 'FontSize', 16);
legend('show');

scatter(final_points(:, 1), final_points(:, 2), 30, colors, 'filled', 'DisplayName', ['Фінальні точки']);
